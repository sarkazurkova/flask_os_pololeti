...
class ZaznamKnih(Base):
    __tablename__ = "zaznam_knih"
    id = Column(Integer, primary_key=True)
    nazev = Column(String(100), nullable=False)
    autor = Column(String(100), nullable=False)
    pocet_stran = Column(Integer, nullable=False)
    time = Column(DateTime, nullable=False, server_default=func.now())
    def __repr__(self):
        return f"<ZaznamKnih(nazev='{self.nazev}', autor='{self.autor}', pocet_stran={self.pocet_stran})>"   